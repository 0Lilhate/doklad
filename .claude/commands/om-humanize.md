...
- If charcount matters (review content in `perf/`), verify limits after editing with `node --experimental-strip-types .claude/scripts/charcount.ts <file>`. Output is JSON with `total_chars`, `total_lines`, and per-section breakdown.
