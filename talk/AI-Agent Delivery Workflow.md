...
> [!note]
> Формулировка «активируются автоматически» здесь — упрощение для intro. На уровне runtime никакой автоматической активации не существует: модель на каждом ходу сама решает, вызвать ли встроенный tool `skill`. Это решение probabilistic, не deterministic, и часто пропускается. Детально — ниже в разделе «OpenCode skills под капот: lifecycle, runtime, orchestration» и в «Почему skills не вызываются автоматически».
...
До этого был словарь. Дальше — как это реально работает. Этот раздел рассчитан на senior dev, AI infrastructure engineer и agent platform engineer. Если вы строите команду на opencode и хотите понимать, почему ваш `pr-reviewer` skill в половине случаев не зовётся — это для вас.
...
> [!important]
> Если вы пришли из ML-frameworks (LangChain, LangGraph, LlamaIndex), у вас привычка думать о skill как о «функции с registered handler». Это ложная модель. Skill ближе к **сохранённой главе README**, которую агент решит дочитать. Никакой execution, никакой callback — только текст, попадающий в context модели в нужный момент.
...
function initializeSkills(): SkillRegistry {
  ...
  const paths = [
    expandHome("~/.config/opencode/skills"),
    ...walkUpToGitRoot(cwd(), ".opencode/skills"),
    ...walkUpToGitRoot(cwd(), ".claude/skills"),
  ];

  for (const dir of paths) {
    for (const skillDir of listDirs(dir)) {
      const skillMdPath = join(skillDir, "SKILL.md");
      if (!exists(skillMdPath)) continue;

      // Читаем ТОЛЬКО frontmatter. Тело остаётся на диске.
      const frontmatter = parseYamlFrontmatter(readFirstBytes(skillMdPath, 4096));

      if (!frontmatter.name || !frontmatter.description) {
        log.warn(`Skipping skill ${skillDir}: missing name/description`);
        continue;
      }
      ...
      registry.register({
        ...
        path: skillMdPath,      // body stays on disk
        sourceDir: skillDir,
      });
    }
  }
...
После init модель **ещё не знает** ни о каких skills. Знание появится, когда opencode соберёт **каталог tools** для следующего turn'а.
...
Pseudo-code сборки tool description:

```typescript
function buildSkillToolDescription(registry: SkillRegistry): ToolDefinition {
  const listingXml = registry.all()
    .map(s => `  <skill name="${s.name}" description="${escapeXml(s.description)}"/>`)
    .join("\n");
...
When the user request matches one of the descriptions above, call
skill(name="...") BEFORE proceeding with other tools. The skill content
contains step-by-step instructions you should follow.
`,
    parameters: {
...
- **Бюджет на весь листинг** в Claude Code — около 1% context window (для 200k context — ~2000 токенов). В opencode — свой лимит, тот же порядок.
...
Сессия с 50 зарегистрированными skills:
  ...
  → opencode/Claude Code обрезают descriptions
